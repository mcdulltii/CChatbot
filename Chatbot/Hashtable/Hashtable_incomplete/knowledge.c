...
void append(hashtable_t *weights, hashtable_t *hashtable, hashtable_t *intents, char *intent, char *entity, char *response) {
	...
	entry_t *findintent = ht_get(intents, intent);
	...
	sprintf(stringNum, "%d", weightCount);
	if (lowerWord != NULL) {
		ht_set(weights, lowerWord, stringNum);
	}
	free(lowerWord);
	sprintf(stringNum2, "%d", intentCount*1000+weightCount);
...
{
	append(weights, hashtable, intents, strdup(intent), strdup(entity), strdup(response));
	return KB_FOUND;
}
...
int knowledge_read(hashtable_t *weights, hashtable_t *hashtable, hashtable_t *intents, FILE *f)
{
	...
	while (!feof(f)) {
		...
		if (!strstr(string, "=") && !isAlpha) {
			...
			for (int i=0; i<cnt+1; i++) {
				...
				if (intent == NULL && i==cnt) {
					...
					cnt++;
					break;
				} else if (intent->value == charstring) {
		...
		// Reads file for Entity = Response
			...
			knowledge_put(weights, hashtable, intents, intend, entity, response);
		}
	}
	return 0;
}
