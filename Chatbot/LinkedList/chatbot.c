...
{
	FILE *f;
	/* The loop checks for the [filename]. If no filename is specified, the program will prompt the user to retype*/
	if (inc >= 3)
	{
		f = fopen(inv[2], "r");
...
int chatbot_do_save(int inc, char *inv[], char *response, int n)
{
	...
	int i;
	FILE *f;
	/*Check for [filename]. If the user did not indicate a filename to save, the chatbot will prompt the user*/
	if (inv[1] != NULL)
	{
...
int chatbot_do_smalltalk(int inc, char *inv[], char *response, int n)
{
	...
	/*Once the verb has been found, the following loop will store the items back into a variable*/
	int check = 0;
	char *space = " ";
