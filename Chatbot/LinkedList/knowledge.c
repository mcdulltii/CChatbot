...
/* In order to get the response, a linear probing method has been employed. Once found, it will return the response
 * accordingly. If not found, a number will be return where the chatbot will prompt the user
 * 
...
int knowledge_get(const char *intent, const char *entity, char *response, int n)
{
	...
	/*If the intent is located, a linear probe is done to match the entity. Once found, it will output the response accordingly*/
	if (compare_token(intent, "who") == 0)
	{
		/*A pointer to the head of the link list is used here. In order to match the entity, a linear probing method is used*/
		struct LIST *ptr = who_head;
		while (ptr != NULL)
		{	
			/*If an exact match is provided, it will return response and KB_FOUND accordingly*/
			if (compare_token(ptr->entity, entity) == 0)
			{
				snprintf(response, n, "%s", ptr->response);
				return KB_FOUND;
			}
			/*Else it will search by keyword, if keyword is found, it will return response and KB_FOUND accordingly*/
			else
			{
...
int knowledge_read(FILE *f)
{
	...
	/*This while-loop will proceed to import the ini file line by line. Every line will be checked for the following formation
	 *such as '[', ']' or '='. Once the line is extracted and formatted, the program will proceed to 
	 *create nodes to form link list
	 */ 
	while (fgets(mander, MAX_ENTITY + 1 + MAX_RESPONSE + 1, f))
	{
		mander[strcspn(mander, "\n")] = 0;
		/*A for-loop to check every characters for '[' ,']' or '='*/
		for (int i = 0; i < strlen(mander); i++)
		{
			/*If the line does not contain an '=', the function will recognize it as an intent and will proceed to remove '[' and ']'*/
			if (isalpha(mander[i]) != 0 && !strstr(mander, "="))
			{
				char *charptr = strtok(mander, delim1);
				char *charmander = strtok(charptr, delim2);
				/*Once an intent that matches anything below, a value will be assigned to 'num' */
			...
			/*Once the program determine that the line is not an intent, it will proceed to recognize this as a response*/
			else if (isalpha(mander[i]) == 0 && strstr(mander, "="))
			{
				char *entity = strtok(mander, delim3);
				char *response = strtok(NULL, delim3);
				/*Once formatting is done, it will form the data into nodes*/
				switch (num)
				{
				case 0:
